    ...
    <v-flex>
      ...
      >
        ...
        <template v-slot:item.opciones="{ item }">
          ...
          <v-icon small @click="mostrarComprobante(item)">print</v-icon>
      ...
      <!-- Comprobante -->
      <v-dialog v-model="comprobanteM" max-width="1000px">
        <v-card>
          <v-card-title class="headline">
            <v-btn text @click="crearPDF()">
              ...
              <section>
                <div>
                  <table id="facarticulo">
                    ...
                    <tbody>
                      <tr v-for="detalle in detalles" :key="detalle._id">
          ...
          <v-card-actions>
            <v-btn color="red darken-1" @click="ocultarComprobante()">
              Cancelar
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    mostrarComprobante(item) {
      ...
      this.comprobanteM = 1;
      this.auxNum = item.num_comprobante;
    },
    ocultarComprobante() {
      this.comprobanteM = 0;
    },
    crearPDF() {
      ...
      html2canvas(quotes).then(function (canvas) {
        ...
        doc.save("ComprobanteVenta#" + aux + ".pdf");
      });
    },
    limpiar() {
      this.persona = "";
    ...
    // Valida datos del formulario antes de enviar
    validar() {
      this.valida = 0;
      this.validaMensaje = [];
      if (!this.persona) {
    ...
    //obtiene la lista de proveedores dentro de un select
    selectProve() {
      let me = this;
      var proveeArray = [];
      let header = {
    ...
    //Obtiene los artículos por nombre o descripción
    listarArticulos() {
      let me = this;
    ...
    //Envia los datos para registrarlos
    guardar() {
      let me = this;
      let header = {
        token: this.$store.state.token,
